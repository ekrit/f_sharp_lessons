...
// Implements IHtmlParser to parse HTML contents from URL
let webParser =
    { new IHtmlParser with
        member this.ParseHtml url = HtmlDocument.Load(url) }


// implements IHtmlParser to Parse HTML contents from local file
let fileParser =
    { new IHtmlParser with
        member this.ParseHtml filepath = 
            filepath
...
// parse wirh webParser
parseHtml webParser "https://github.com/dotnet/fshaprp"

// read local file contents and parse with fileParser
Path.Join(__SOURCE_DIRECTORY__, "fshapr-github-repo.html")
